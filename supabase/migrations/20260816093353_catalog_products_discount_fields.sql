-- Storefront catalog: discount fields for Home Featured / Top Sales.
...
comment on view public.catalog_products is
  'Storefront catalog row: product + display price/variant + variant_count + review aggregates + name_normalized + discount fields for Featured.';

comment on column public.catalog_products.has_discount is
  'True when any variant has current_price < original_price.';

comment on column public.catalog_products.discount_depth is
  'Max (original - current) / original across variants; 0 when none are discounted.';
