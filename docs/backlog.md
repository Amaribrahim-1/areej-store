...
| **List pagination beyond the storefront catalog** | `/products` already pages in the database (`?page=`, page size 12). Everything else is a full fetch or a cap, not a pager: customer order history (all of that customer's orders); admin orders / products / reviews / contact messages (RPCs return the whole list — admin order filter/sort is client-side on that list); product-detail reviews capped at 20 with no “load more”; Home Latest / Featured / Testimonials are fixed slices. When order or catalog volume grows, those lists will get slow and the admin screens will get too long. Paginate (or cursor) in the query — do not fetch all rows and slice in JS. |
