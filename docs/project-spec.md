...
14. **Categories:** originally a fixed four-value check (`Perfumes`, `Musk`, `Fermentation`, `Hair Oil`) with admin CRUD deferred. **Pulled into Phase 13:** `categories` table + Alaa can add a category from the product form. The four seed rows remain; no delete (products FK restrict). Catalog filter reads from the table.
