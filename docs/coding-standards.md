...
- Adding a new product category should only touch `features/products/` (plus a `categories` insert — Alaa can add a label from the admin product form). Catalog filters and admin list read from that table; cart/checkout stay unaware. No category delete in MVP (products FK is restrict).
