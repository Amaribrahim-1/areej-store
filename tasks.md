...
- [x] **15.3 — Security review**: run the checklist in `code-quality-security.mdc` end to end. Verify RLS by attempting cross-user reads (customer A reading customer B's order), confirm no service-role key in any client bundle, confirm admin routes reject a non-admin session server-side.
      **Done:** Checklist verified: RLS default-deny still holds (customer A cannot read customer B's orders/items/profile; anon sees no orders; admin RPCs raise `NOT_ADMIN` for a customer). Guest `/admin` (and nested admin pages) 307 to `/admin/login?next=…` via `requireAdmin` in the protected layout — not a client hide. No `SERVICE_ROLE` / service-role key in source, `.env.example`, or client Supabase helpers (anon key only). Server-side totals, Zod-before-write, and review/contact sanitization already in place. Hardening: revoked `TRUNCATE` from `anon`/`authenticated` (TRUNCATE bypasses RLS) and leftover INSERT/DELETE on `orders` / writes on `order_items` (place_order SECURITY DEFINER is the only writer). Advisor WARN leftovers (intentional SECURITY DEFINER RPCs + leaked-password protection) stay for **15.5**.
- [x] **15.4 — Performance pass**: confirm every product image goes through `next/image`, check for query waterfalls, review `staleTime` per resource, check bundle size. Note where the React Compiler already handles memoization — manual `useMemo`/`useCallback` should be near-absent (`code-quality-security.mdc`).
      **Done:** Product photos already used `next/image` (no storefront `<img>`). No `useMemo`/`useCallback`/`React.memo` — React Compiler covers it. `staleTime` already per-resource (catalog 5m, admin products 60s, categories 5m, customer orders 2m, admin orders/KPIs 30s, reviews 2m, cart 2m); admin reviews magic `60 * 1000` now `ADMIN_REVIEWS_STALE_TIME_MS`. Fixes: Cairo weights 400–700 only (drop unused 200/300/800/900); BrandLogo `sizes="64px"` so the 1024px source is not treated as 100vw; catalog first two cards `priority` + 2/3-col `sizes`; home Featured/Testimonials static imports so their queries start with Latest (no extra JS chunk delay); customer layout `Suspense` around the navbar `getCurrentUser()` so the page RSC is not blocked on auth; `images.formats` avif+webp. Featured listing still does one extra batched variants round-trip after catalog ids (not N+1) — left as-is; TanStack server prefetch stays backlog.
- [x] **15.5 — Supabase advisors re-run** + fix findings.
      **Done:** Security: the 5 SECURITY DEFINER WARNs (`list_home_testimonials`, `list_product_reviews`, `submit_contact_message`, `list_admin_reviews`, `place_order`) are the same intentional pattern accepted since 1.8 — each function's own migration documents why (profiles RLS is owner/admin-only, so these RPCs bridge it deliberately, with an internal `NOT_ADMIN` guard on the admin one). `auth_leaked_password_protection` WARN has no SQL/migration fix — it's an Auth setting only togglable from the Supabase Dashboard (Authentication → Sign In / Providers → Password → "Leaked password protection"); no MCP/API tool exposes it here, so it's parked for Alaa/the developer to flip by hand. Performance: fixed `multiple_permissive_policies` on `reviews` DELETE by merging `reviews_delete_own` (C.2) + `reviews_delete_admin` (A.1) into one `reviews_delete_own_or_admin` policy (`user_id = auth.uid() OR private.is_admin()`), same access, one policy evaluated instead of two. Remaining `unused_index` INFOs (`order_items`, `orders`, `products`, `reviews`) are expected on a low-row dev project — Postgres prefers seq scans on tiny tables; the indexes exist for scale, not current dev traffic — left in place.
- [x] **15.6 — Empty / loading / error states audit**: every list and every mutation has all three.
      **Done:** Audit of every list + mutation. Catalog category filters now have loading/error/empty (retry, “الكل” still works on error). Catalog empty splits “no products yet” vs “no filter match” with a clear-filters CTA. Add-review “my review” error has retry. Admin category field and dashboard KPI missing-data no longer go blank. Mutation forms expose `aria-busy` while in-flight (matching contact/checkout).
- [x] **15.7 — SEO & metadata**: per-page Arabic metadata, Open Graph image, `robots`, sitemap.
