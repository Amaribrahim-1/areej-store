...
- **Exception:** `docs/walkthroughs/` (areej-teach) — teaching prose in Egyptian Arabic; usage contracts/code/paths stay English. Chat after a walkthrough is a short pointer to `docs/walkthroughs/current.md`.
...
After backend ships, after a full task the developer asked the AI to implement, and after a review: use `.cursor/skills/areej-teach/SKILL.md` (`areej-teach`) so the lesson lives in `docs/walkthroughs/current.md` (overwrite each time): problem + solution, pages/files without internals, flow, key decisions. Reviews: must/should/ذوق problems only, then a bare change list.
